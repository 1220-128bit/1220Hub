...
local Games = {
	...
	[93922097153911] = "loadstring(game:HttpGet(''))()",
}
...
subLabel.Text = "Welcome to 1220Hub"
...
-- // --- เพิ่มข้อความ 3 บรรทัดที่หายไป --- // --
local msg1 = Instance.new("TextLabel")
msg1.Size = UDim2.new(0, 320, 0, 25)
msg1.Position = UDim2.new(0, 130, 0, 115)
msg1.BackgroundTransparency = 1
msg1.Text = "Follow me to use the script"
msg1.TextColor3 = Color3.new(1, 1, 1)
msg1.Font = Enum.Font.GothamBold
msg1.TextSize = 16
msg1.TextXAlignment = Enum.TextXAlignment.Left
msg1.ZIndex = 9
msg1.Parent = mainFrame

local msg2 = Instance.new("TextLabel")
msg2.Size = UDim2.new(0, 320, 0, 25)
msg2.Position = UDim2.new(0, 130, 0, 140)
msg2.BackgroundTransparency = 1
msg2.Text = "Then you will get what you want"
msg2.TextColor3 = Color3.new(1, 1, 1)
msg2.Font = Enum.Font.GothamBold
msg2.TextSize = 16
msg2.TextXAlignment = Enum.TextXAlignment.Left
msg2.ZIndex = 9
msg2.Parent = mainFrame

local msg3 = Instance.new("TextLabel")
msg3.Size = UDim2.new(0, 320, 0, 20)
msg3.Position = UDim2.new(0, 130, 0, 170)
msg3.BackgroundTransparency = 1
msg3.Text = "ทุกสคริปต์มีสิทธ์โดนแบน โปรดใช้อย่างระมัดระวังนะจั้ฟ"
...
local function makeBtn(text, pos, color)
    ...
    btn.MouseButton1Down:Connect(function()
        ...
        btn:TweenSize(UDim2.new(0, 95, 0, 32), "Out", "Quad", 0.1, true)
    end)
    btn.MouseButton1Up:Connect(function()
        btn:TweenSize(UDim2.new(0, 100, 0, 35), "Out", "Back", 0.1, true)
    end)
    
    return btn
end

local closeBtn = makeBtn("Cl0se", UDim2.new(1, -225, 1, -55), Color3.fromRGB(60, 20, 100))
local nextBtn = makeBtn("Next →", UDim2.new(1, -115, 1, -55), MainColor)

RunService.Heartbeat:Connect(function(dt)
    bgGrad.Rotation = (bgGrad.Rotation + dt * 15) % 360
    spinner.Rotation = (spinner.Rotation + dt * 200) % 360
    if math.random(1, 8) == 1 then createSpark(mainFrame) end
end)
...
local function finish(callback)
    ...
    local t = TweenService:Create(loadingFrame, TweenInfo.new(0.6, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
        Size = UI_SIZE,
        BackgroundTransparency = 1
    })
    t:Play()
...
nextBtn.MouseButton1Click:Connect(function()
    ...
    finish(function()
        screenGui:Destroy()
        pcall(function() loadstring(Games[game.PlaceId])() end)
    end)
end)
